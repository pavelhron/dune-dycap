        ...
        for (int j=0; j<n; j++) // loop over columns
          {
            ...
            for (int i=0; i<m; i++)
              {
                if ((up(lfsv,i)-down(lfsv,i))/delta > limit)
                  {
                    //std::cout << "bv in jacobian volume " << (up(lfsv,i)-down(lfsv,i))/delta << std::endl;
                    //std::exit(0);
                  }
        ...
        for (int j=0; j<n_s; j++)
          {
            ...
            for (int i=0; i<m_s; i++)
              {
                ...
                if ((up_s(lfsv_s,i)-down_s(lfsv_s,i))/delta > limit)
                  {
                    //std::cout << "bv in jacobian skeleton i " << i << " " << (up_s(lfsv_s,i)-down_s(lfsv_s,i))/delta << " " << up_s(lfsv_s,i) << " " <<  down_s(lfsv_s,i)<< std::endl;
                    //std::exit(0);
                  }
            ...
            for (int i=0; i<m_n; i++)
              {
                if ((up_n(lfsv_n,i)-down_n(lfsv_n,i))/delta > limit)
                  {
                    // std::cout << "bv in jacobian skeleton i " << i << " " << (up_s(lfsv_s,i)-down_s(lfsv_s,i))/delta << " " << up_s(lfsv_s,i) << " " <<  down_s(lfsv_s,i)<< std::endl;
                    //std::exit(0);
                  }
