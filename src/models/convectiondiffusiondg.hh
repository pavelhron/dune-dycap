...
namespace Dune {
  namespace PDELab {
     ...
     *
     * \f{align*}{
     *   \nabla\cdot(-A(x) \nabla u + b(x) u) + d(x)u &=& f \mbox{ in } \Omega,  \\
        ...
        for (typename Dune::QuadratureRule<DF,dim>::const_iterator it=rule.begin(); it!=rule.end(); ++it)
          {
            ...
            typename T::Traits::RangeFieldType c = param.d(eg.entity(),it->position());
    ...
    template<typename T>
    class DGTemporal : public NumericalJacobianApplyVolume<DGTemporal<T> >,
    ...
    public:
      ...
      DGTemporal (T& tp, int intorder_=2,double scaling=1.0)
        : param(tp), intorder(intorder_)
        , _scaling(scaling)
      {}
        ...
        for (typename Dune::QuadratureRule<DF,dim>::const_iterator it=rule.begin(); it!=rule.end(); ++it)
          {
            ...
            // u*phi_i
            RF factor = _scaling * it->weight() * eg.geometry().integrationElement(it->position())*param.c(eg.entity(),it->position());
            for (size_type i=0; i<lfsu.size(); i++)
	...
	param.setTime(t);
      ...
      {
        return std::numeric_limits<typename T::Traits::RangeFieldType>::max(); //initial value should be big enough
      }
