        ...
        for (int j=0; j<n; j++) // loop over columns
          {
            ...
            for (int i=0; i<m; i++)
              mat.rawAccumulate(lfsv,i,lfsu,j,(up(lfsv,i)-down(lfsv,i))/(2.*delta));
            u(lfsu,j) = x(lfsu,j);
          }
      }
        ...
        for (int j=0; j<n_s; j++)
          {
            ...
            asImp().alpha_skeleton(ig,lfsu_s,u_s,lfsv_s,lfsu_n,u_n,lfsv_n,downview_s,
                                   downview_n);
            u_s(lfsu_s,j) += 2.0*delta;
            asImp().alpha_skeleton(ig,lfsu_s,u_s,lfsv_s,lfsu_n,u_n,lfsv_n,upview_s,
                                   upview_n);

            for (int i=0; i<m_s; i++)
              mat_ss.accumulate(lfsv_s,i,lfsu_s,j,(up_s(lfsv_s,i)-down_s(lfsv_s,i))/(2.*delta));
            for (int i=0; i<m_n; i++)
              mat_ns.accumulate(lfsv_n,i,lfsu_s,j,(up_n(lfsv_n,i)-down_n(lfsv_n,i))/(2.*delta));
        ...
        for (int j=0; j<n_n; j++)
          {
            ...
            asImp().alpha_skeleton(ig,lfsu_s,u_s,lfsv_s,lfsu_n,u_n,lfsv_n,downview_s,
                                   downview_n);
            u_n(lfsu_n,j) += 2.0*delta;
            asImp().alpha_skeleton(ig,lfsu_s,u_s,lfsv_s,lfsu_n,u_n,lfsv_n,upview_s,
                                   upview_n);
            for (int i=0; i<m_s; i++)
              mat_sn.accumulate(lfsv_s,i,lfsu_n,j,(up_s(lfsv_s,i)-down_s(lfsv_s,i))/(2.*delta));
            for (int i=0; i<m_n; i++)
              mat_nn.accumulate(lfsv_n,i,lfsu_n,j,(up_n(lfsv_n,i)-down_n(lfsv_n,i))/(2.*delta));
        ...
        for (int j=0; j<n_s; j++)
          {
            ...
            for (int i=0; i<m_s; i++)
              mat_ss.rawAccumulate(lfsv_s,i,lfsu_s,j,(up_s(lfsv_s,i)-down_s(lfsv_s,i))/(2.0*delta));
            u_s(lfsu_s,j) = x_s(lfsu_s,j);
          }
      }
        ...
        for (int j=0; j<n; j++) // loop over columns
          {
            ...
            asImp().alpha_volume(eg,lfsu,u,lfsv,upview);
            for (int i=0; i<m; i++)
              y.rawAccumulate(lfsv,i,((up(lfsv,i)-down(lfsv,i))/(2.*delta))*x(lfsu,j));
            u(lfsu,j) = x(lfsu,j);
          }
      }
        ...
        for (int j=0; j<n_s; j++)
          {
            ...
            asImp().alpha_skeleton(ig,lfsu_s,u_s,lfsv_s,lfsu_n,u_n,lfsv_n,upview_s,
                                   upview_n);
            for (int i=0; i<m_s; i++)
              y_s.accumulate(lfsv_s,i,((up_s(lfsv_s,i)-down_s(lfsv_s,i))/(2.*delta))*x_s(lfsu_s,j));
            for (int i=0; i<m_n; i++)
              y_n.accumulate(lfsv_n,i,((up_n(lfsv_n,i)-down_n(lfsv_n,i))/(2.*delta))*x_s(lfsu_s,j));
            u_s(lfsu_s,j) = x_s(lfsu_s,j);
          }
        ...
        for (int j=0; j<n_n; j++)
          {
            ...
            asImp().alpha_skeleton(ig,lfsu_s,u_s,lfsv_s,lfsu_n,u_n,lfsv_n,upview_s,
                                   upview_n);
            for (int i=0; i<m_s; i++)
              y_s.accumulate(lfsv_s,i,((up_s(lfsv_s,i)-down_s(lfsv_s,i))/(2.*delta))*x_n(lfsu_n,j));
            for (int i=0; i<m_n; i++)
              y_n.accumulate(lfsv_n,i,((up_n(lfsv_n,i)-down_n(lfsv_n,i))/(2.*delta))*x_n(lfsu_n,j));
            u_n(lfsu_n,j) = x_n(lfsu_n,j);
          }
      }
        ...
        for (int j=0; j<n_s; j++)
          {
            ...
            u_s(lfsu_s,j) -= delta;
            asImp().alpha_boundary(ig,lfsu_s,u_s,lfsv_s,downview_s);
            u_s(lfsu_s,j) += 2.*delta;
            asImp().alpha_boundary(ig,lfsu_s,u_s,lfsv_s,upview_s);
            for (int i=0; i<m_s; i++)
              y_s.rawAccumulate(lfsv_s,i,((up_s(lfsv_s,i)-down_s(lfsv_s,i))/(2.*delta))*x_s(lfsu_s,j));
            u_s(lfsu_s,j) = x_s(lfsu_s,j);
          }
      }
